...
<form class="custom-comments comment-form" method="post" id="commentform">
	...
	<?php else : ?>
		<?php if ( $this->options['name']['placeholder'] ) : ?>
			<div class="form-item comment-form-author">
				...
				<input type="text" placeholder="<?php echo $this->options['name']['placeholder']; ?>" 
														   <?php
															echo $this->options['name']['required'] ? 'required' : '';
															?>
				 name="name">
			</div>
		<?php endif; ?>
		<?php if ( $this->options['email']['placeholder'] ) : ?>
			<div class="form-item comment-form-email">
				...
				<input type="email" placeholder="<?php echo $this->options['email']['placeholder']; ?>" 
															<?php
															echo $this->options['email']['required'] ? 'required' : '';
															?>
				 name="email">
			</div>
		<?php endif; ?>
	...
	if ( $this->options['additional_fields'] ) :
		...
		<div class="form-item">
					...
					if ( $item['label'] ) :
						?>
						<label><?php echo $item['label']; ?></label> <?php endif; ?>
			<?php
			if ( $item['slug'] ) :
				?>
				<input type="text" placeholder="<?php echo $item['slug']; ?>"
												 name="<?php echo $item['slug']; ?>"> <?php endif; ?>
		</div>
			<?php
	endforeach;
endif;
	...
	<?php if ( $this->options['text']['placeholder'] ) : ?>
		<div class="form-item comment-form-comment">
			...
			<textarea id="custom_comments_text"
					  placeholder="<?php echo $this->options['text']['placeholder']; ?>"
											  <?php
												echo $this->options['text']['required'] ? 'required' : '';
												?>
			 name="text"></textarea>
		</div>
	<?php endif; ?>
