...
class Custom_Comments_Admin {
	...
	public function add_menu() {
		...
		if ( empty( $admin_page_hooks[ $parent_menu_slug ] ) ) {
			add_menu_page( $parent_menu_name, $parent_menu_name, 'manage_options', $parent_menu_slug,
				array( $this, 'render_plugin_page' ) );
		}
		add_submenu_page( $parent_menu_slug, 'Custom Comments', 'Custom Comments', 'manage_options', $this->plugin_name,
	...
	public function plugin_settings_page() {
		...
		} else if ( $_GET['custom_comment'] && $_GET['action'] == 'delete' ) {
			//$this->update_comment($data);
			$id = $_GET['custom_comment'] . '_' . $_GET['type'];
			$this->delete_comment( $id );
		} else if ( $_GET['custom_comment'] && $_GET['action'] == 'approve' ) {
			$data = array( 'comment_approved' => '1' );
			$this->update_comment( $data );
		} else if ( $_GET['custom_comment'] && $_GET['action'] == 'unapprove' ) {
			$data = array( 'comment_approved' => '0' );
			$this->update_comment( $data );
		} else if ( $_POST['action'] == 'edited_custom_comment' ) {
			$this->update_comment( $_POST );
		} else if ( $_GET['custom_comment'] && $_GET['action'] == 'spam' ) {
			$data = array( 'comment_approved' => 'spam' );
			$this->update_comment( $data );
		} else if ( $_GET['custom_comment'] && $_GET['action'] == 'trash' ) {
			$data = array( 'comment_approved' => 'trash' );
			$this->update_comment( $data );
		} else if ( $_GET['action'] == 'move_to_trash' ) {
			$data = array( 'comment_approved' => 'trash' );
			$this->update_comment( $data );
